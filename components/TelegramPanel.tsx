...
export interface TelegramSignalData {
  sigTime:     number;   // timestamp du bar signal (dédup auto-send)
  symbol:      string;
  tf:          string;
  type:        "buy" | "sell";
  score:       string;
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                <div style={{ background:"rgba(99,102,241,.08)", border:"1px solid rgba(99,102,241,.3)", borderRadius:10, padding:"14px 16px", fontSize:12, lineHeight:1.9 }}>
                  ...
                  <div style={{ color:"#94a3b8" }}>
                    1. Ouvre{" "}
                    <a href={`https://t.me/${botInfo?.username ?? ""}`} target="_blank" rel="noreferrer" style={{ color:"#818cf8", fontWeight:700 }}>
                ...
                <div style={{ background:"rgba(245,158,11,.06)", border:"1px solid rgba(245,158,11,.25)", borderRadius:10, padding:"14px 16px", fontSize:12, lineHeight:1.9 }}>
                  ...
                  <div style={{ color:"#94a3b8" }}>
                    ...
                    3. Envoie un message dans le canal<br />
                    4. Clique à nouveau sur <strong style={{ color:"#f59e0b" }}>Vérifier</strong>
                  </div>
                </div>
              </div>
            )}
...
interface Props {
  ...
  autoSend:    boolean;                         // envoi auto activé depuis la page
  onAutoSent?: (score: string) => void;         // callback quand envoi auto réussi
}

export default function TelegramPanel({ signal, autoSend, onAutoSent }: Props) {
  const [token,      setToken]     = useState("");
  const [chatId,     setChatId]    = useState("");
  const [ready,      setReady]     = useState(false); // localStorage chargé
  const [showConfig, setShowConfig]= useState(false);
  const [sending,    setSending]   = useState(false);
  const [status,     setStatus]    = useState<"idle"|"ok"|"err">("idle");
  const [errDetail,  setErrDetail] = useState("");

  // Dernier sigTime auto-envoyé (évite le double envoi sur chaque refresh)
  const lastAutoSentRef   = useRef<number>(0);
  // Premier signal reçu = signal déjà connu, on ne l'envoie pas au démarrage
  ...
  // ── Auto-send : déclenché quand sigTime change (nouveau signal détecté) ──
  useEffect(() => {
    if (!ready || !autoSend || !signal || !token || !chatId) return;

    // Premier signal au démarrage → juste l'enregistrer, ne pas envoyer
    ...
    // Même signal qu'avant → pas d'envoi
    if (signal.sigTime === lastAutoSentRef.current) return;

    // Nouveau signal → envoyer
    lastAutoSentRef.current = signal.sigTime;
  ...
  return (
    <>
      <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap", background:"#0d0d1a", border:"1px solid #1c1c38", borderRadius:10, padding:"6px 10px" }}>
        {/* Pastille état */}
        <span style={{ width:7, height:7, borderRadius:"50%", display:"inline-block", flexShrink:0, background: isConfigured ? "#22c55e" : "#ef4444", boxShadow: isConfigured ? "0 0 6px #22c55e70" : "none" }} />
        <span style={{ fontSize:11, fontWeight:700, color:"#475569" }}>📨</span>

        {/* Auto-send badge */}
        {isConfigured && autoSend && (
          <span style={{ fontSize:9, fontWeight:700, color:"#22c55e", background:"rgba(34,197,94,.1)", border:"1px solid rgba(34,197,94,.25)", borderRadius:4, padding:"1px 6px" }}>
            AUTO
          </span>
        )}
          ...
          <>
            ...
            {signal ? (
              <button onClick={() => sendSignal(signal, true)} disabled={sending} style={{
                fontSize:11, fontWeight:700, padding:"4px 12px", borderRadius:6, cursor: sending ? "wait" : "pointer",
