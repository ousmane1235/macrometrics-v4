...
export default function SignalChart({ yfSymbol, label, tfIdx, onResult }: Props) {
  ...
  // Fetch + auto-refresh toutes les 30 secondes (sans skeleton sur les refreshes silencieux)
  useEffect(() => {
    let mounted = true;
    let tick    = 30;
    ...
    const doFetch = (initial: boolean) => {
      ...
      fetch(`/api/chart-data?symbol=${encodeURIComponent(yfSymbol)}&interval=${tf.yfInterval}&range=${tf.yfRange}`)
        ...
        .then((d: Candle[]) => {
          ...
          setRefreshing(false);
          tick = 30; setCountdown(30);        // réinitialiser le compte à rebours
        })
        .catch(() => {
          if (!mounted) return;
    ...
    doFetch(true);
    const refreshId   = setInterval(() => doFetch(false), 30_000);
    const countdownId = setInterval(() => {
      tick = tick <= 1 ? 30 : tick - 1;
